...
import { Check, ChevronDown, Pencil, Plus, Trash2, Wand2 } from 'lucide-react'
...
export default function EditorStylesPanel({
  ...
  const [editingStyles, setEditingStyles] = useState<boolean>(false)
  const [editingPresets, setEditingPresets] = useState<boolean>(false)
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', padding: '0.75rem' }}>
      <Section title="Academic Presets" count={allPresets.length}>
        ...
        <PanelFooter
          ...
          onToggleEdit={() => setEditingPresets((v) => !v)}
        />
        {editingPresets && <CustomPresetForm onAdd={addCustomPreset} />}
      </Section>
      ...
      <Section title="Document Styles" count={allStyles.length}>
        ...
        <PanelFooter
          ...
          onToggleEdit={() => setEditingStyles((v) => !v)}
        />
        {editingStyles && <CustomStyleForm onAdd={addCustomStyle} />}
      </Section>
    </div>
  )
}
...
/** Row of expand/edit controls shown under each section body. */
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginTop: '0.375rem', padding: '0 0.375rem' }}>
      ...
      <button
        ...
        style={{
          ...
          fontSize: '0.6875rem',
          fontWeight: 600,
          color: editing ? 'var(--foreground)' : 'var(--muted-foreground)',
          backgroundColor: 'transparent',
          border: 'none',
          cursor: 'pointer',
          fontFamily: 'inherit',
      ...
      >
        <Pencil style={{ width: '12px', height: '12px' }} /> Edit custom
      </button>
    </div>
  )
}
